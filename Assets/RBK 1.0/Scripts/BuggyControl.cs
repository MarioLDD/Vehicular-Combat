    ...
    void FixedUpdate()
    {
        ...
        if (activeControl)
        {
            if (controlMode == ControlMode.simple)
            {
                ...
                {
                    steer = Mathf.MoveTowards(steer, userInput.ControllerInputX, 0.2f);
                    accel = userInput.ControllerInputY;
                    brake = userInput.ControllerInputHandBrake;
                    shift = userInput.ControllerInputNitro;
                }
